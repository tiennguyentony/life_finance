...
`TAX_SERVICE_URL` is required locally only. On Vercel, a private service binding generates a deployment-aware URL from the `tax_service` entry in [`vercel.json`](vercel.json) and injects it into the web service. The tax API remains protected by `TAX_SERVICE_TOKEN`. See [`docs/operations/vercel-services.md`](docs/operations/vercel-services.md).
