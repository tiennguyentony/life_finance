...
`TAX_SERVICE_URL` is required locally only. On Vercel the tax service is deployed at `/svc/tax` inside this same project, and its URL is generated from the `tax_service` entry in [`vercel.json`](vercel.json). The route remains protected by `TAX_SERVICE_TOKEN`. See [`docs/operations/vercel-services.md`](docs/operations/vercel-services.md).
